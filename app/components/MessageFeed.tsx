  ...
  return (
    <>
      {messages ? (
        messages.map((message, index) => {
          ...
          return (
            <div className="flex flex-row" key={message._id}>
              <div
                className={`w-fit mb-2 p-4 border max-w-[420px] rounded border-gray-300 ${message.author === currentUser ? "bg-[var(--yellow)] ml-auto" : "bg-white"} ${isAuthor && !isPreviousAuthor ? "mt-2" : "mt-0"}`}
              >
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 bg-gray-50 border border-gray-200 rounded-lg p-3 mx-auto mb-2">
          No messages yet
        </div>
      )}
    </>
